...
function setup() {
  ...
  pixelDensity(2);
//z  pg = createGraphics(1200, 1200, SVG);
...
function spiral3D(c) {
  ...
  for (let i = 0; i < 3150; i += 0.5) {
    let r = i / 5.9 +
      (max(0, i / 450) * cos(radians(i / 15 ))) * sin(radians(i * 3* 4)) +
      (max(0, i / 230) * cos(radians(i / 20 ))) * sin(radians(i * 3 * 4));
...
function spiral3D1(c) {
  ...
  for (let i = 0; i < 3150; i += 0.5) {
    let r = i / 5.9 +
      (max(0, i / 450) * cos(radians(i / 15 ))) * sin(radians(i * 3* 4)) +
      (max(0, i / 230) * cos(radians(i / 10 ))) * sin(radians(i * 3 * 4));
    let x = -r * cos(radians(i * 4));
    let y = r * sin(radians(i * 4));
     pg1.vertex(x, y);
  }
