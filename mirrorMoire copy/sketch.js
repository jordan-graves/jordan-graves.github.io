...
function spiral3D(c) {
  ...
  for (let y = 0; y < pg.width * 2; y += inc) {
    ...
    for (let x = -pg.height; x < pg.height*2; x++) {
      ...
      let s = 0.8*2;
      let t = 0.4*2;
      let r = a * 12 * sin(radians(y * 0 * s + x * 1 * t)) 
          * sin(radians(x * 0 * t + y * 1 * s + 0))  
      * sin(radians(x * -2/3 * t + y * -3/40 * s + 180)) 
    //  * sin(radians(x * -2/4 * t + y * 2/3 * s + 0)) 
...
//  plane(800, 480 * 800 / 640);
  ...
  let r = 200-150*sin(radians(frameCount/2));
   r=100;
  // Second Layer
  push();
  texture(pg);
