...
function preload() {
   ...
   screenNumber=int(getParameterByName("sn"));
  ...
  if (screenNumber == 1) {
  bg = loadImage("SBMM Background-01.png");
  bg1 = loadImage("SBMM Midground-01.png");
  bg2 = loadImage("SBMM Foreground-01.png");
  } else if (screenNumber == 2) {
  bg = loadImage("SBMM Background-02.png");
  bg1 = loadImage("SBMM Midground-02.png");
  bg2 = loadImage("SBMM Foreground-02.png");
  } else {
  bg = loadImage("SBMM Background-03.png");
  bg1 = loadImage("SBMM Midground-03.png");
  bg2 = loadImage("SBMM Foreground-03.png");
  }
...
function setup() {
  ...
  if (screenNumber ==1) {
  k = new Stalk(150, 715, 1240);
  k1 = new Stalk(300, 730, 350);
  k2 = new Stalk(200, 745, 1350);
  k3 = new Stalk(500, 900, 50);
  positionsX = [66,53,93,97,151,130,173,133,503,564,607,537,456,362,422,503,586,644,605,535,44,115,70,22,123,185,232,160,80,28,122,201,301,263,157,72,368,781,844,921,884,809,935,843,776,887,836,801,899,936];
    positionsY = [91,143,149,203,212,234,273,299,285,287,316,320,323,326,346,354,352,357,395,376,377,386,416,444,430,420,465,465,465,496,488,496,475,519,522,516,523,287,289,300,331,326,357,359,391,395,434,474,448,482];
foreground = 20;
schoolSize = 20;
  } else if (screenNumber ==2) {
...
schoolSize = 80;
  ...
  k = new Stalk(100, 715, 340+550);
  k1 = new Stalk(300, 730, 500 - 50+550);
  k2 = new Stalk(200, 745, 50);
  k3 = new Stalk(500, 900, 150);
  positionsX = [648,698,669,613,712,670,575,635,720,684,592,542,636,739,780,866,938,918,827,767,873,927,822,745,873,678,804,916,853,751,716,799,927,858,762,666,719,809,904,180,106,31,89,41,142,185,107,162,57,133];
  positionsY = [86,90,120,143,148,166,184,201,195,228,224,258,250,244,242,241,226,273,273,305,302,336,332,356,363,414,381,403,427,403,443,438,462,481,477,490,517,508,508,311,302,334,352,382,365,382,419,432,469,475];
  foreground = 14;
  schoolSize = 50;
}
...
function draw() {
  ...
  for (let i = 0; i < whale.length; i++) {
    push();
    translate((screenNumber-1)*-960,0);
    whale[i].display();
    pop();
    if (whale[i].x < -200 || whale[i].x > 960*3 +150) {
     // whale.splice(i, 1);
      i--;
    }
  }
...
class Whale {
  constructor(sf) {
    ...
    if (int(Date.now()/100000)%10<5) {
      this.fish = sf || random(allDolphin);
      this.petal = whaleImg;
      this.x = 960*3+150;
      this.y = map(int(Date.now()/10000) * 7 % 9, 0, 8, 40, 160);
      this.speed = -0.5 / 2 *10;
      ...
      this.petal = whaleImg;
      this.x = -150;
      this.y = map(int(Date.now()/10000) * 7 % 9, 0, 8, 40, 160) + 20;
      this.speed = -1 * -0.5 / 2 * 10;
      this.size = map(this.y, 0, 400, 1, 2);
      this.direction = "right";
      totalDolphin++;
    }
  ...
  display() {
    if (this.direction == "left") {
            this.x = 960*3+400 -(Date.now()/100 % (960*3+400))-200;
    if (this.x > screenNumber*960+ 200 || this.x < (screenNumber-1)*960- 200) {
      return;
    }
      ...
      pop();
        ...
        this.x = (Date.now()/100 % (960*3+400))-200;
            if (this.x > screenNumber*960+ 200 || this.x < (screenNumber-1)*960- 200) {
      return;
    }
      push();
...
class Diver {
  ...
  display() {
   // this.x += this.speed;
    this.x = 960*3+200 -(Date.now()/120 % (960*3+200))-200;
    push();
    translate(this.x, this.y);
    //scale(this.size);
...
class Boid {
  constructor(x, y) {
    ...
    if (screenNumber == 3) {
          this.velocity = createVector(-1, random(-0.5, 0.5));
          this.position = createVector(900, y);
    }
  ...
  flock(boids) {
    ...
    this.applyForce(new p5.Vector(abs(a) * sin(frameCount / 10 + screenNumber*120), a));
    let b = map(this.position.x, 0, 960, 0.025, -0.025);
    this.applyForce(new p5.Vector(b, abs(b) * sin(frameCount / 10 * screenNumber)));
  }
