...
slider = createSlider(-PI/2,PI/2,0,0.1);
slider.parent("color1-section")
  // --- Create dropdowns ---
  sel1 = createSelect();
  sel2 = createSelect();
  sel3 = createSelect();
...
translate(-200,0)
  ...
  let angleX = map(width/2+150*sin(radians(frameCount/1/2)), 0, width, -PI / 2, PI / 2, true);
  
   let angleY = map(width/2+50*sin(radians(frameCount/1.5/2)), 0, width, -PI / 2, PI / 2,true);

      angleX = map(mouseX, 0, width, -PI / 2, PI / 2, true);
  
    angleY = map(mouseY, 0, width, -PI / 2, PI / 2,true);

    angleY = 0;
    angleX = slider.value()*-1;
      let offset = 70*sin(radians(frameCount/1.5/2));
...
// }
  ...
  noStroke();
  texture(sw_pg); // Apply thesw_pg buffer as a texture
