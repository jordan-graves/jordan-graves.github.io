...
let bg, bg1, bg2;
let positionsX = [
  40,84,53,100,57,137,99,47,156,112,64,125,175,181,121,57,790,770,826,873,840,778,745,819,909,892,858,789,749,828,903,531,490,470,416,369,46,111,64,127,48,780,804,838,869,901,912,928,852,180,351,416,387,314,463,525,434,359,482,276,403,323,
];
let positionsY = [
  46,64,91,116,140,162,173,185,206,218,241,269,264,315,301,304,111,166,160,155,202,213,263,248,195,244,278,298,335,322,309,394,345,381,371,368,414,422,460,480,508,496,440,478,427,503,454,402,521,500,430,430,468,479,461,486,496,505,517,499,518,520,
];
...
let jsonStarfish, jsonDolphin, jsonGaribaldi;

let socket, port;
function preload() {
  	allStarfish = loadJSON("../starfish.json");
  	allDolphin = loadJSON("../dolphin.json");
  	allGaribaldi = loadJSON("../garibaldi.json");
...
function setup() {
  ...
  divers = createGraphics(800, 800);
	...
	socket.on('starfish', newStarfish);
	socket.on('disableStarfish', disableStarfish);
	socket.on('enableStarfish', enableStarfish);
  	socket.on('dolphin', newDolphin);
	socket.on('disableDolphin', disableDolphin);
	socket.on('enableDolphin', enableDolphin);
socket.on('garibaldi', newGaribaldi);
	socket.on('disableGaribaldi', disableGaribaldi);
	socket.on('enableGaribaldi', enableGaribaldi);
...
function gotData(data) {
  ...
  for (let i = allStarfish.starfish.length; i < keys.length; i++) {
    let key = keys[i];

    let thisProject = myProjects[key];
    allStarfish.starfish.push(thisProject);
  }
  if (loaded) {
    for (let i = 0; i < 3; i++) {
      let a = Math.floor(random(0, positionsX.length));
      ...
      let x = positionsX[a];
      let y = positionsY[a];
      let sf = allStarfish.starfish[allStarfish.starfish.length - 1];
      taken[a] = true;
...
function gotDolphinData(data) {
  ...
  for (let i = allDolphin.dolphin.length; i < keys.length; i++) {
    let key = keys[i];

    let thisProject = myProjects[key];
    allDolphin.dolphin.push(thisProject);
  }
  if (dolphinLoaded) {
    for (let i = 0; i < 1; i++) {
      let sf = allDolphin.dolphin[allDolphin.dolphin.length - 1];
...
function gotGaribaldiData(data) {
  ...
  for (let i = allGaribaldi.garibaldi.length; i < keys.length; i++) {
    let key = keys[i];

    let thisProject = myProjects[key];
    allGaribaldi.garibaldi.push(thisProject);
  }
  if (garibaldiLoaded) {
    for (let i = 0; i < 1; i++) {
      let sf = allGaribaldi.garibaldi[allGaribaldi.garibaldi.length - 1];
...
function mousePressed() {
  ...
  let sf = int(random(allStarfish.starfish.length));
  for (let i = 0; i < 3; i++) {
...
if (!sf) {
  sf = int(random(allStarfish.starfish.length))
}
this.id = sf;
    // if(!mute) {
		// 	let s = random(fishSounds);
    ...
    this.fish = allStarfish.starfish[sf] || random(allStarfish.starfish);
  ...
  display() {
      if (allStarfish.starfish[this.id].blocked) {
      return;
    }
...
class Dolphin {
  constructor(sf) {
    if (!sf) {
  sf = int(random(allDolphin.dolphin.length))
}
    this.id = sf;
    if(!mute) {
			let s = random(fishSounds);
    ...
    if (Math.random() < 0.5) {
      this.fish = allDolphin.dolphin[sf] || random(allDolphin.dolphin);
      this.petal = loadImage(this.fish.fish);
  ...
  display() {
      if (allDolphin.dolphin[this.id].blocked) {
      return;
    }
    this.x += this.speed;
...
class Whale {
  constructor(sf) {
    ...
    if (Math.random() < 0.5) {
      this.fish = sf || random(allDolphin.dolphin);
      this.petal = whaleImg;
...
class Diver {
  ...
  divers.rotate(radians(-60)-diverAngle/4);
  divers.translate(-80, -5);
  ...
  divers.rotate(radians(-40)+diverAngle/4);
  divers.translate(-5, -5);
  ...
  divers.rotate(radians(-80)+diverAngle/4);
  divers.translate(-75, -5);
...
class Garibaldi {
  constructor(sf) {
    if (!sf) {
  sf = int(random(allGaribaldi.garibaldi.length))
}
    this.id = sf;
    if(!mute) {
			let s = random(fishSounds);
    ...
    if (Math.random() < 0.5) {
      this.fish = allGaribaldi.garibaldi[sf] || random(allGaribaldi.garibaldi);
      this.petal = loadImage(this.fish.fish);
  ...
  display() {
    if (allGaribaldi.garibaldi[this.id].blocked) {
      return;
    }
    this.x += this.speed;
    if (this.direction == "left") {
...
class Stalk {
  ...
  display() {
    ...
    let j = 0;
    for (let i = this.start; i < this.end; i += this.offset) {
      let a = map(i, this.start, this.end, 40, 0);
      let y = i;
      let x =
      this.x + a * cos(i / 3 + fc / 3*13+this.x) + a * cos(i + fc * 1*13);
      vertex(x, y);
      this.myKelp[j].x = x;
      j++;
    }
...
function disableStarfish(data) {
	console.log("disable sf"+data)
    if (data.date == allStarfish.starfish[data.number].date)
	allStarfish.starfish[data.number].blocked = true;
    else {
      for (let i=0; i<allStarfish.starfish.length; i++) {
            if (data.date == allStarfish.starfish[i].date) {
              	allStarfish.starfish[i].blocked = true;
            }
      }
    }
}

function enableStarfish(data) {
	console.log("enable sf"+data)
  if (data.date == allStarfish.starfish[data.number].date)
	allStarfish.starfish[data.number].blocked = false;
 else {
      for (let i=0; i<allStarfish.starfish.length; i++) {
            if (data.date == allStarfish.starfish[i].date) {
              	allStarfish.starfish[i].blocked = false;
            }
      }
    }
}

function disableDolphin(data) {
	console.log("disable d"+data)
    if (data.date == allDolphin.dolphin[data.number].date)
	allDolphin.dolphin[data.number].blocked = true;
 else {
      for (let i=0; i<allDolphin.dolphin.length; i++) {
            if (data.date == allDolphin.dolphin[i].date) {
              	allDolphin.dolphin[i].blocked = true;
            }
      }
    }
}

function enableDolphin(data) {
	console.log("enable d"+data)
      if (data.date == allDolphin.dolphin[data.number].date)
	allDolphin.dolphin[data.number].blocked = false;
 else {
      for (let i=0; i<allDolphin.dolphin.length; i++) {
            if (data.date == allDolphin.dolphin[i].date) {
              	allDolphin.dolphin[i].blocked = false;
            }
      }
    }
}

function disableGaribaldi(data) {
	console.log("disable g"+data)
      if (data.date == allGaribaldi.garibaldi[data.number].date)
	allGaribaldi.garibaldi[data.number].blocked = true;
 else {
      for (let i=0; i<allGaribaldi.garibaldi.length; i++) {
            if (data.date == allGaribaldi.garibaldi[i].date) {
              	allGaribaldi.garibaldi[i].blocked = true;
            }
      }
    }
}

function enableGaribaldi(data) {
	console.log("enable g"+data)
        if (data.date == allGaribaldi.garibaldi[data.number].date)
	allGaribaldi.garibaldi[data.number].blocked = false;
 else {
      for (let i=0; i<allGaribaldi.garibaldi.length; i++) {
            if (data.date == allGaribaldi.garibaldi[i].date ) {
              	allGaribaldi.garibaldi[i].blocked = false;
            }
      }
    }
}
...
function newStarfish(data) {
	allStarfish.starfish.push(data);
	//convertJSONFlowers();
    ...
    for (let i = 0; i < 3; i++) {
      ...
      myStarfish.push(new Starfish(a, x, y, allStarfish.starfish.length-1));
    }
        if(!mute) {
			let s = random(fishSounds);
	   s.play();
	...
	console.log("new starfish");
}

function newDolphin(data) {
	allDolphin.dolphin.push(data);
	//convertJSONFlowers();
	//flowers.push(new GeneratedFlower(json.flowers.length-1, random(0,width), random(0,height)));

      myDolphin.push(new Dolphin(allDolphin.dolphin.length-1));

	console.log("new dolphin");
}

function newGaribaldi(data) {
	allGaribaldi.garibaldi.push(data);
	//convertJSONFlowers();
	//flowers.push(new GeneratedFlower(json.flowers.length-1, random(0,width), random(0,height)));

      myGaribaldi.push(new Garibaldi(allGaribaldi.garibaldi.length-1));

	console.log("new garibaldi");
}