...
class Kelp {
  ...
  display() {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      let y =
        0 +
        10 * sin(i * this.frame + this.offset1) +
        10 * sin(i * this.frame * this.freq + this.offset2);
      if (i == 1) {
        y *= 1.5;
        stroke(this.stem);
        line(x, y, 0, 0);
    ...
    noStroke();
    beginShape();
    curveVertex(points[0].x - 10, points[0].y);

    curveVertex(points[0].x - 10, points[0].y);
    for (let i = 0; i < points.length; i++) {
      let t = map(i, 0, points.length, 10, 0);
      curveVertex(points[i].x + 10, points[i].y - t);
    }
    ...
    curveVertex(points[0].x - 10, points[0].y);
    for (let i = 0; i < points.length; i++) {
      let t = map(i, 0, points.length, -2, 0);
      curveVertex(points[i].x + 10, points[i].y - t);
    }
    for (let i = points.length - 1; i >= 0; i--) {
      let t = map(i, 0, points.length, -10, 0);
